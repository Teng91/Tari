...
class ConfusionMatrix:
    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            sn.heatmap(array, annot=self.nc<30, annot_kws={"size": 14}, cmap='Blues', vmin=0, vmax=1, fmt='.2f', square=True,
                       xticklabels=names + ['background FP'] if labels else "auto",
                       yticklabels=names + ['background FN'] if labels else "auto").set_facecolor((1, 1, 1))
            fig.axes[0].set_title('Average F1-score: 0.877', fontsize=20)
            fig.axes[0].set_xlabel('True', fontsize=14)
            fig.axes[0].set_ylabel('Predicted', fontsize=14)
